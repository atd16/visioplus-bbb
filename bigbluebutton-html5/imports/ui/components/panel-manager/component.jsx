...
import VoteContainer from '/imports/ui/components/vote/container';
...
// Variables for resizing vote.
const VOTE_MIN_WIDTH = 320;
const VOTE_MAX_WIDTH = 400;
...
class PanelManager extends Component {
  ...
  renderVote() {
    return (
      <div className={styles.vote} key={this.voteKey}>
        <VoteContainer vote={this.props.vote} adminInfos={this.props.adminInfos} isGreenlightConnected={this.props.isGreenlightConnected} makeGreenlightCall={this.props.makeGreenlightCall} publishVote={this.props.publishVote} stopVote={this.props.stopVote} />
      </div>
    );
  }

  renderVoteResizable() {
    const { voteWidth } = this.state;
    const { isRTL } = this.props;
    ...
    return (
      <Resizable
        minWidth={VOTE_MIN_WIDTH}
        maxWidth={VOTE_MAX_WIDTH}
        ref={(node) => { this.resizableVote = node; }}
        enable={resizableEnableOptions}
        key={this.voteKey}
        size={{ width: voteWidth }}
        onResizeStop={(e, direction, ref, d) => {
          window.dispatchEvent(new Event('resize'));
          this.setState({
            voteWidth: voteWidth + d.width,
          });
        }}
      >
        {this.renderVote()}
  ...
  render() {
    ...
    if (openPanel === 'vote') {
      if (enableResize) {
        panels.push(this.renderVoteResizable());
      } else {
        panels.push(this.renderVote());
      }
    }
